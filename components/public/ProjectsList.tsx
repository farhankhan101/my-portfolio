...
export default function ProjectsList({ initialProjects }: ProjectsListProps) {
  ...
  // Dynamically extract unique categories from initialProjects database values
  const categories = ['All', ...Array.from(new Set(initialProjects.map((p) => p.category))).sort()]
            ...
            <div className="flex justify-center pt-6">
              {visibleCount < filteredProjects.length ? (
                <button
                  ...
                  className="px-6 py-2.5 rounded-full text-xs font-bold bg-sky-600 hover:bg-sky-500 dark:bg-sky-500 dark:hover:bg-sky-400 text-white border border-transparent shadow-md hover:shadow-sky-550/20 transition-all duration-300 hover:-translate-y-0.5 cursor-pointer flex items-center gap-2"
                >
