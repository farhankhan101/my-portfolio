      ...
      {value ? (
        ...
        <div className="relative group rounded-lg overflow-hidden border border-border bg-secondary/40 aspect-video max-w-md">
          <Image
            ...
            unoptimized={value.startsWith('/uploads/')} // skip next/image optimization for local uploads folder to avoid missing next.config.js domains issues
          />
          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
            <button
              type="button"
              onClick={removeImage}
              className="p-2 bg-red-600 hover:bg-red-500 text-white rounded-full transition-colors cursor-pointer shadow-lg"
              title="Remove image"
            >
              <X size={18} />
            </button>
            ...
            <div className="flex flex-col items-center text-center space-y-2">
              ...
              <div>
                <p className="text-sm font-medium text-foreground">
                  <span className="text-sky-500 dark:text-sky-400 hover:underline">Click to upload</span> or drag and drop
                </p>
